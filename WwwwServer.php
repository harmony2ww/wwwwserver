 ...
 * @license    https://github.com/X0xx-1110/WWWW-Server/blob/main/LICENSE
 * @version    [1.8.1]
...
* All the methods are inside one class, that would create possible these 
* functionality.
**/
class WwwwServer
{
    ...
    private array $_securityFilesWeb = ["", "index.php", "index.html", 
    "index.htm"];
    private string $_strSecureMsg = "";
    
    /**
    * Ccmmon function about web server it all.
    * Basic and common calculation of a source are here. Connections and 
    * sockets. Many functions about parsing and functionality.
    * @return bool 
    **/
    public function handle(int $port, string $webDirectoryOfUse) : bool
    {
        $numRequests = 0;
        $countRequests = 0;
        if (isset($webDirectoryOfUse) && ! empty($webDirectoryOfUse) 
        	&& strlen($webDirectoryOfUse) > 1 && is_dir($webDirectoryOfUse)) {
        ...
        $this->_socket = stream_socket_server($this->_protocol."://"
        	.$this -> _address.":".$port, $errno, $errstr);
        ...
        if ( ! isset($this -> _socket) || empty($this -> _socket) 
            ...
            for(;;) {
                $this -> _connection = stream_socket_accept($this -> _socket,
                 -1);
                if (isset($this -> _connection) 
                	&& ! empty($this -> _connection)) {
                    $gatheredRequest = stream_socket_recvfrom(
                    	$this->_connection, 1000000, STREAM_PEEK);
                    //set start time
                    $this -> _timestampStart = microtime();
                    //parse Request
                    $requestArray = $this -> _preParseRequestToArray(
                    	$gatheredRequest);
                    ...
                    fwrite($this -> _connection, $this -> _headersResponce.
                    	$this -> _responce);
                    //Write to console
                    if ($regExCheck === false) {
                            ...
                            if ($this -> _isError === true) {
                                ...
                                if ($this -> _responce === false) {
                                    $this -> _strSecureMsg = "[ FAULT SECURITY
                                     check! ]";
                                    $this -> _log("security._log",
                                     $numRequests, $this -> $requestArray[0]
                                     ."[ FAULT SECURITY check! ]");
                                }
                                print "  |".(++$numRequests)."|".
                                $this->_timeDelta().
                                "delta|====================>".$requestArray[0].
                                "  ".$this->_strSecureMsg."\n";
                            }
                        }
                    ++$countRequests;
                    $this -> _log("request._log", $countRequests,
                     trim($requestArray[0])."----".trim($requestArray[0]));
                    $this -> _log("user_agent._log", $countRequests,
                     trim($requestArray[0])."----".trim($requestArray[5]));
                    $this -> _log("reffer._log", $countRequests,
                     trim($requestArray[0])."----".trim($requestArray[11]));
                    $this -> _log("content_type._log", $countRequests,
                     trim($requestArray[0])."----".trim($this -> _contentType));
                    $this -> _log("length_responce._log", $countRequests,
                     trim($requestArray[0])."----".trim(
                     	$this -> _contentLength));
                    $this -> _log("response._log", $countRequests,
                     trim($requestArray[0])."----".trim(
                     	$this -> _responceHeaders["HTTP/1.1"]));
                    $this -> _log("request_data._log", $countRequests,
                     trim($requestArray[0])."----".trim(
                     	$this -> _responceNoGzip));
    ...
    **/
    ...
    { 
        ...
        if ($this -> _securityFilesWeb === true) {
            if (isset($this -> _securityFilesWeb) 
            	&& ! empty($this -> _securityFilesWeb) 
            	&& is_array($this -> _securityFilesWeb) 
            	&& count($this -> _securityFilesWeb)) {
    ...
    * @return void
    **/
    private function _checkRequestSecurity(array $requestArray,
     bool $regExStatus) : void
    {
    ...
    **/
    ...
    {
        $temporaryHeaders="";
        if (isset($this -> _responceHeaders) 
        	&& ! empty($this -> _responceHeaders) 
        	&& is_array($this -> _responceHeaders)) {
            foreach ($this -> _responceHeaders as $firstPart=>$secondPart) {
                $temporaryHeaders = $temporaryHeaders.$firstPart." "
                .$secondPart;
            }
        }
        if (isset($temporaryHeaders) && ! empty($temporaryHeaders) 
        	&& is_string($temporaryHeaders) && strlen($temporaryHeaders) > 1) {
    ...
    **/
    ...
    {
        if (isset($this -> _timestampEnd) && ! empty($this -> _timestampEnd) 
        	&& isset($this -> _timestampStart) 
        	&& ! empty($this -> _timestampStart)) {
            return (string) abs(number_format(floatval(substr(
            	$this -> _timestampEnd, 0, 9))-floatval(substr(
            		$this -> _timestampStart, 0, 9)), 4, ".", ""));
        }
    }
    /**
    ...
    **/
    ...
    {
        if (isset($request) && ! empty($request) && is_string($request) 
        	&& strlen($request) > 1) {
            $gatheredRequestArray = explode("\n", $request);
        }
        if (isset($gatheredRequestArray) && ! empty($gatheredRequestArray) 
        	&& is_array($gatheredRequestArray) 
        	&& count($gatheredRequestArray) > 1) {
    ...
    **/
    private function _preParseRequest(string $request) : string
    {
        if (isset($request) && ! empty($request) && is_string($request) 
        	&& strlen($request) > 1) {
            $gatheredRequestArray = explode("\n", $request);
            $firstLineByRequestFirst = 
            $gatheredRequestArray[array_key_first($gatheredRequestArray)];
        }
        if (isset($firstLineByRequestFirst) 
        	&& ! empty($firstLineByRequestFirst) 
        	&& is_string($firstLineByRequestFirst) 
        	&& strlen($firstLineByRequestFirst) > 1) {
    ...
    **/
    ...
    {
        $this -> _setResponceHeaders("HTTP/1.1",
         $this -> _statusResponce."\r\n");
        ...
        $this -> _setResponceHeaders("Content-Type:",
        ...
        $this -> _setResponceHeaders("Content-Length:",
         $this -> _contentLength."\r\n");
    ...
    **/
    ...
    {    
        ...
        if( strpos($firstLineRquestSecondParam, "?") !== false) {
            $deltaLen = strpos($firstLineRquestSecondParam, "?") 
            - strpos($firstLineRquestSecondParam, ".");
            $extension = substr($firstLineRquestSecondParam,
             strpos($firstLineRquestSecondParam, "."), $deltaLen);
        } else {
                    $extension = substr($firstLineRquestSecondParam,
                     strpos($firstLineRquestSecondParam, "."));
                }
    ...
    * @return void
    **/
    private function _setResponceHeaders(string $nameRespondHeader,
     string $valueRespondHeader) : void
    {
    ...
    * @return bool
    **/
    private function _fileWrite(string $filename, int $id,
     string $message) : bool
    {
        if(isset($filename) && ! empty($filename) && is_string($filename) 
        	&& strlen($filename) > 4 && isset($message) && ! empty($message) 
        	&& is_string($message) && strlen($message) > 1) {
            touch($filename);
            $handling = fopen($filename, "a+");
            fwrite($handling,  "[".$id."]-[".date(DATE_RFC2822)."]-"
            	.$message."\n");
            fclose($handling);
            return true;
        }
    ...
    **/
    private function _parseRequest(array $req) : array
    {
            ...
            if (isset($dataRequestFirstRecord ) 
            	&& ! empty($dataRequestFirstRecord) 
            	&& $dataRequestFirstRecord  == "GET") {
            ...
            if (isset($dataRequestFirstRecord) 
            	&& ! empty($dataRequestFirstRecord) 
            	&& $dataRequestFirstRecord == "POST") {
                $requestedUrl = substr($dataRequestSecondRecord, 1);
                $temporaryGet = $req[array_key_last($req)];
            }
            if (strpos($requestedUrl, "./") === false 
            	&& strpos($requestedUrl, "../") === false) {
                if (isset($temporaryGet) && ! empty($temporaryGet)) {
                    return array("x_file"=>$requestedUrl,
                     "x_data_REQUEST"=>$temporaryGet,
                      "x__protocol"=>$dataRequestFirstRecord);
                } else {
                    return array("x_file"=>$requestedUrl,
                     "x_data_REQUEST"=>"",
                      "x__protocol"=>$dataRequestFirstRecord);
    ...
    **/
    private function _securityCheck(string $urlAboutCheck) : bool
    {
        ...
        if (isset($arrayCheck) && ! empty($arrayCheck) 
        	&& is_array($arrayCheck) && count($arrayCheck)) {
    ...
    **/
    private function _securityCheckWebFiles(string $request) : bool
    {
        ...
        if (isset($filename) && ! empty($filename) 
        	&& is_string($filename) && strlen($filename) > 1 
        	&& $this -> _securityFilesWebStatues === true 
        	&& in_array($filename, $this -> _securityFilesWeb)) {
            return true;
        }
    ...
    **/
    ...
    {
        if(isset($temporaryUri["x_file"]) 
        	&& ! empty($temporaryUri["x_file"]) 
        	&& ! is_null($temporaryUri["x_file"])) {
        ...
        if ($this -> _securityArrayStatuses === true 
        	&& isset($xFile) && !empty($xFile) 
        	&& $this->_securityCheck($xFile) === false) {
            $this -> _status = "406 406";
            return false;
        }
        if ($this -> _securityFilesWebStataStuses === true 
        	&& isset($xFile) && !empty($xFile) 
        	&& $this->_securityCheckWebFiles($xFile) === false) {
            $this -> _status = "406 406";
            return false;
        }
        ...
        if(isset($temporaryUri["x__protocol"]) 
        	&& ! empty($temporaryUri["x__protocol"]) 
        	&& $temporaryUri["x__protocol"] != "HEAD" 
        	&& $temporaryUri["x__protocol"] != "PING") {
            
            if (isset($xFile) && ! empty($xFile) && strlen($xFile) > 1) {
                if (strpos($xFile, "php") !== false 
                	&& $this -> phpVersion === "8.0") {
                    $this -> _isError = false;
                    $this -> _status = "200 OK";
                    return shell_exec("/usr/bin/php8.0  -r ' "
                    	.$this -> _webVariables($xFile,
                    	 $temporaryUri["x__protocol"],
                    	  $temporaryUri["x_data_REQUEST"] ).
                    	" include_once(\"".$xFile."\"); ' ");
                }
                elseif (strpos($xFile, "php") !== false 
                	&& $this -> phpVersion === "7.4") {
                    $this -> _isError = false;
                    $this -> _status = "200 OK";
                    return shell_exec("/usr/bin/php7.4 -r ' "
                    	.$this -> _webVariables($xFile,
                    	 $temporaryUri["x__protocol"],
                    	  $temporaryUri["x_data_REQUEST"])
                    	." include_once(\"".$xFile."\"); ' ");
                ...
                elseif (strpos($xFile, "php") !== false 
                    ...
                    $this -> _isError = false;
                    $this -> _status = "200 OK";
                    return shell_exec("/usr/bin/php7.0 -r ' "
                    	.$this -> _webVariables($xFile,
                    	 $temporaryUri["x__protocol"],
                    	  $temporaryUri["x_data_REQUEST"])
                    	." include_once(\"".$xFile."\"); ' ");
                ...
                elseif (strpos($xFile, "php") !== false 
                    ...
                    $this -> _isError = false;
                    $this -> _status = "200 OK";
                    return shell_exec("/usr/bin/php -r ' ".
                    	$this -> _webVariables($xFile,
                    	 $temporaryUri["x__protocol"],
                    	  $temporaryUri["x_data_REQUEST"]).
                    	" include_once(\"".$xFile."\"); ' ");
                } else {
                    if( isset($xFile) && !empty($xFile) &&
                     strpos($xFile, ".")!==false && strlen($xFile) > 3 &&
                      is_file($xFile) && is_readable($xFile) ) {
                        $this -> _status = "200 OK";
        ...
        if(isset($temporaryUri["x__protocol"]) 
        	&& ! empty($temporaryUri["x__protocol"]) 
        	&& $temporaryUri["x__protocol"] == "HEAD") {
            //@TODO: later functionality of starting a function that calculate 
            //the HEAD protocul starting functions.
            $this -> _status = "200 OK";
            return "";
        }
        if(isset($temporaryUri["x__protocol"]) 
    ...
    * @return array
    **/
    private function _parseWebGetVars(string $_protocol,
     string $webVars) : array
    {
        $newArr = array();
        $newArrNotReturned = array();
        if (isset($_protocol) && ! empty($_protocol) 
        	&& ($_protocol == "GET" || $_protocol == "POST" 
        		|| $_protocol == "HEAD")) {
            if (isset($webVars) && ! empty($webVars) 
            	&& is_string($webVars) && strlen($webVars) > 1) {
                $parsedVars = explode("&", $webVars);
                if (isset($parsedVars) && ! empty($parsedVars) 
                	&& is_array($parsedVars) && count($parsedVars)) {
        ...
        if (isset($_protocol) && ! empty($_protocol) 
        	&& $_protocol == "PINGSERVICE") {
            //@TODO
        }
        if (isset($_protocol) && ! empty($_protocol) 
        	&& $_protocol == "LOOKUPSERVICE") {

        }
        if (isset($_protocol) && ! empty($_protocol) 
        	&& $_protocol == "ROUTESERVICE") {
    ...
    * @return string
    **/
    private function _webVariables(string $file, string $_protocol,
     string $webVars) : string
    {
        $arrayWebVars = array();
        $arrayWebVars = $this -> _parseWebGetVars($_protocol, $webVars);
        $strPhpCodeOne = " ";
        if (isset($arrayWebVars) && ! empty($arrayWebVars) 
        	&& is_array($arrayWebVars) && count($arrayWebVars)) {
            foreach ($arrayWebVars as $keyVar => $webVar) {
                $strPhpCodeOne .= "\$_".strtoupper($_protocol).
                "[\"".$keyVar."\"]=\"".$webVar."\"; \n";
                $strPhpCodeOne .= "\$_REQUEST[\"".$keyVar."\"]=\"".
                $webVar."\"; \n";
            }
        }
    ...
    **/
    private function _log(string $file, int $id, string $message) : bool
    {
        ...
        if($this->_fileWrite(getcwd()."/".$this -> _directory_log.
        	$file, $id, $message)) {
            return true;
        }
        return false;
